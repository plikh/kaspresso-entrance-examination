...
class CerealStorageImplTest {
    ...
    fun `exception when can not add additional container for new kind of cereal`() {
        ...
        val exception =
            org.junit.jupiter.api.assertThrows<IllegalStateException> { storage.addCereal(Cereal.MILLET, 25f) }
    ...
    fun `get difference of cereal when there is no particular container`() {
        ...
        assertTrue(Cereal.BUCKWHEAT in storage.mainStorage.keys)
    ...
    fun `get zero value and check cereal added to container`() {
        ...
        val cerealAmount = storage.mainStorage[Cereal.BUCKWHEAT]
    ...
    fun `exception when storage does not contain passed cereal type`() {
        val exception =
            org.junit.jupiter.api.assertThrows<NoCerealException> { storage.getCereal(Cereal.MILLET, 0.098f) }
    ...
    fun `get true when remove empty container`() {
        ...
        assertFalse(storage.mainStorage.keys.contains(Cereal.PEAS))
    }
    ...
    fun `exception when checking free space and can not add new container`() {
        ...
        val exception =
            org.junit.jupiter.api.assertThrows<IllegalStateException> { storage.getSpace(Cereal.MILLET) }
